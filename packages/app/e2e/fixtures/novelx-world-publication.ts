...
async function worldVisualFixture(
  ...
  const mapBaseTaskId = "image-world-map"
  const draft = {
    ...
    tasks: [
      ...
      {
        id: "image-world-map-geography-selected",
        type: "map" as const,
        subtype: "region-highlight" as const,
        mapRole: "variant" as const,
        layer: "geography" as const,
        entityId: entity.id,
        baseTaskId: mapBaseTaskId,
        ownerEntityId: entity.id,
        status: "attached" as const,
        title: `${entity.name}地理选中状态`,
        prompt: "保持共享地图不变，只强调赫利俄斯同步环的自然地理范围。",
        rationale: "地理命中后切换完整状态图，泰森区域保持隐藏。",
        sourceEntityIds: [entity.id],
        sourceSha256s: [sourceSha256],
        targetPath: `World/Media/maps/geography/${entity.id}.png`,
        mime: "image/png" as const,
        assetSha256: "6".repeat(64),
        model: "openai-compatible/gpt-image-2",
        startedAt: 1700000004200,
        completedAt: 1700000004250,
        errorCode: null,
      },
      {
        id: "image-world-map-human-selected",
        type: "map" as const,
        subtype: "region-highlight" as const,
        mapRole: "variant" as const,
        layer: "human" as const,
        ...
        ownerEntityId: entity.id,
        status: "attached" as const,
        title: `${entity.name}人文选中状态`,
        prompt: "保持共享地图不变，只强调赫利俄斯同步环的人文疆域。",
        rationale: "人文命中后切换完整状态图，泰森区域保持隐藏。",
        sourceEntityIds: [entity.id],
        sourceSha256s: [sourceSha256],
        targetPath: `World/Media/maps/human/${entity.id}.png`,
        mime: "image/png" as const,
        assetSha256: "7".repeat(64),
        model: "openai-compatible/gpt-image-2",
        startedAt: 1700000004250,
        completedAt: 1700000004275,
        errorCode: null,
      },
      {
