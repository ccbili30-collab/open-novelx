...
it.instance(
  "growth editor can only plan, dispatch stage editors, checkpoint memory, and finish the world",
  () =>
    Effect.gen(function* () {
...
it.instance("story editor splits the existing visual tool branch instead of a cover editor", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(story, "novelx_read_story_character")).toBe("allow")
    ...
    expect(story?.prompt).toContain("exactly one short three-chapter novel")
    expect(story?.prompt).toContain("Pass `historyBooks: []` and `references: []`")
    expect(story?.prompt).toContain("task_id")
    expect(story?.prompt).toContain("commit its Markdown directly")
    expect(story?.prompt).toContain("Never create a second Writer")
    expect(story?.prompt).toContain("Do not create JSON receipts")

    const writer = yield* load((svc) => svc.get("novelx-story-writer"))
    expect(evalPerm(writer, "write")).toBe("deny")
    expect(evalPerm(writer, "task", "general")).toBe("deny")
    expect(writer?.prompt).toContain("single long-lived NovelX novel Writer")
    expect(writer?.prompt).toContain("Return Markdown only")
    expect(writer?.prompt).toContain("Chapters two and three continue")
    expect(writer?.prompt).toContain("Do not append JSON")
