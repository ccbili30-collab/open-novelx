...
// etc.).
//
// The callback page follows NovelX desktop tokens while remaining fully
// self-contained for loopback authorization flows.
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `NovelX is now connected to ${escapeHtml(provider)}.` : "NovelX is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `NovelX couldn't finish connecting to ${escapeHtml(provider)}.`
        : "NovelX couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from NovelX.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · NovelX</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("NovelX couldn't finish connecting to "+PROVIDER+"."):"NovelX couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from NovelX."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("NovelX is now connected to "+PROVIDER+"."):"NovelX is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .wordmark { color: var(--oc-text-strong); font-size: 1.125rem; font-weight: 650; letter-spacing: 0; }
...
const WORDMARK = `<span class="wordmark" aria-label="NovelX">NovelX</span>`
